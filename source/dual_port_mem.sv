/////////////////////////////////////////////////////////////////////////////////////////
// Module        : Dual Port Memory (dual_port_mem)
...
module dual_port_mem #(
  ...
  always_ff @(posedge clk_i) begin
    ...
    if (we_i) begin
      for (int i = 0; i < NUM_ROW_BYTES; i++) begin
        if (wstrb_i[i]) begin
          mem_array[waddr_i[ADDR_WIDTH-1:$clog2(NUM_ROW_BYTES)]][i*8+:8] <= wdata_i[i*8+:8];
        end
      end
    end
  end
  ...
  // Asynchronous read
  always_comb rdata_o = mem_array[raddr_i[ADDR_WIDTH-1:$clog2(NUM_ROW_BYTES)]];
