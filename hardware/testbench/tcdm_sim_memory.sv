...
module tcdm_sim_memory (
  ...
  function void write_8(int addr, logic [7:0] data);
    mem[addr] = data;
  endfunction

  function logic [7:0] read_8(int addr);
    return mem[addr];
  endfunction

  function void write_16(int addr, logic [15:0] data);
    write_8(addr, data[7:0]);
    write_8(addr + 1, data[15:8]);
  endfunction

  function logic [15:0] read_16(int addr);
    return {read_8(addr + 1), read_8(addr)};
  endfunction

  function void write_32(int addr, logic [31:0] data);
    write_8(addr, data[7:0]);
    write_8(addr + 1, data[15:8]);
    write_8(addr + 2, data[23:16]);
    write_8(addr + 3, data[31:24]);
  endfunction

  function logic [31:0] read_32(int addr);
    return {read_8(addr + 3), read_8(addr + 2), read_8(addr + 1), read_8(addr)};
  endfunction
