...
The generic memory module is a parameterized System verilog memory module. It uses demultiplexer, flipflip(storage), multiplexer to control the memory. The memory only supports aligned access, which means that the address must be a multiple of the data width.
...
| addr_i  | input     | logic | ADDR_WIDTH   | Address input (shared for read and write) |
| we_i    | input     | logic | 1            | Write enable                              |
| wdata_i | input     | logic | DATA_WIDTH   | Write data                                |
