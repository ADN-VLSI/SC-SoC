...
int main() {
  ...
  while ((REG_UART_TXGP & 0x000000FF) != 0x55) {
  ...
  while ((REG_UART_STAT & 0x00100000) == 0x00000000) {
    for (volatile int i = 0; i < 32; i++);
  }
  ...
  // REQUEST RX ACCESS
  REG_UART_RXR = 0x000000AA;
  while ((REG_UART_RXGP & 0x000000FF) != 0xAA) {
    for (volatile int i = 0; i < 32; i++);
  }

  // WAIT FOR RX FIFO TO HAVE ALL 16 BYTES
  // bits[19:10] of STAT = RX FIFO fill level
  while (((REG_UART_STAT & 0x000FFC00) >> 10) != 0x00000010) {
    for (volatile int i = 0; i < 32; i++);
  }

  // CHECK RECEIVED BYTES — parentheses fix operator precedence bug
  int error = 0;
