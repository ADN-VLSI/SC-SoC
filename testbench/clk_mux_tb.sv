...
module clk_mux_tb;
  ...
  task automatic check_t(input string name, ref logic got, ref logic exp);
    bit mismatch;
    repeat (100) begin
      @(exp);
      if (got !== exp) begin
  ...
  // Check Task - compares got vs expected, prints PASS or FAIL
  task check_f(input string name, input logic got, input logic exp);
    if (got !== exp) begin
      $error("FAIL %s", name);
      fail++;
    end else begin
  ...
  task tc_reset();
    arst_ni <= '0;
    #5;
    check_f("TC1: Reset", clk_o, clk_off);
    arst_ni <= '1;
    #20;
  endtask
  ...
  task tc_select_clk0();
    sel_i <= '0;
    #50;
    check_t("TC2: Select clk0", clk_o, clk0_i);
  endtask
  ...
  task tc_select_clk1();
    sel_i <= '1;
    #50;
    check_t("TC3: Select clk1", clk_o, clk1_i);
  endtask
  ...
  task tc_switch_0_to_1();
    ...
    sel_i <= '1;
    #50;
    check_t("TC4: Switch 0->1", clk_o, clk1_i);
  endtask

  // TC5 - clk_o No Unknown Values: check clk_o is not X or Z
  task tc_clk_o_no_unknown();
    #20;
    check_f("TC5: clk_o No Unknown Values", clk_o === 1'bx ? 1'bx : clk_o, clk_o);
  endtask
